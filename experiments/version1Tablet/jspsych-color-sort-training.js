...
jsPsych.plugins['color-sort-training'] = (function() {
  ...
  jsPsych.pluginAPI.registerPreload('activeWord-learning', 'audioTaDa', 'audio');
  jsPsych.pluginAPI.registerPreload('activeWord-learning', 'audioBoing', 'audio');
  ...
  plugin.trial = function(display_element, trial) {
	  ...
	  trial.audioBoing = trial.audioBoing || "stims/boing_1000msSilence.m4a";
			...
			if (trial.feedback) {
				if (isRight==1) {
					...
					im.animate({
						x: centerLocation[0],
						y: centerLocation[1],
					},trial.animationTime/3,mina.easeinout,function() {
						setTimeout(function() {
							endTrial(isRight);
						}, trial.endTrialPause);
					});
