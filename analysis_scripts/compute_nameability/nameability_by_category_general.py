...
def clean_response(response):
    #remove any punctuation/ unusual characters from the response (except apostrophes)
    ...
    return response_punctuation_cleaned

#read in data set
d = pd.read_csv("color_naming_kids.csv")
#check out the data (first 10 rows)
d.loc[range(10),]

#array of unique category names
categoryNames=np.unique(d["color"])
...
for category in categoryNames:
    ...
    for response in d.loc[d["color"]==category,"name"]:
        ...
        response_punctuation_cleaned=clean_response(response)
        #now tokenize
