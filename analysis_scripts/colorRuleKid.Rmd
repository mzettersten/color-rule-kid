...
Analysis walkthrough for the paper “Nameability Supports Rule-based Category Learning in Children and Adults” (authors removed, 2022).
...
kid_subj <-  d %>%
  ...
  group_by(subject,condition, age_m,gender,race, ethnicity,language_history) %>%
  summarize(
...
# Relationship between category learning and color word knowledge {.tabset}
...
We also investigated the proportion of participants who sorted the novel test exemplars into one category or the other, i.e. consistently according to the 100% predictive color feature, or consistently in accordance with the 75% predictive color features. 
...
ggsave(here::here("figures","test_accuracy.png"),width=12, height=6,dpi=600)
```


### Relation between test phase accuracy and color word knowledge

In order to investigate the effect of low nameability color word comprehension on final test accuracy, we fit linear models, separately for adult participants and for child participants, predicting accuracy during the final test phase from the interaction between low nameability color comprehension and condition. 
